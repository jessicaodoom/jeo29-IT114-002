...
//    //UCID: jeo29
//    //DATE: April 13, 2024
    public boolean sendMuteUser(ServerThread mutedUser, ServerThread mutingUser){
        if (currentRoom.isUserMuted(mutedUser.getClientName(),mutingUser.getClientName())) {
            return false; // Don't send mute notification if user is already muted
        }
        currentRoom.muteUser(mutedUser.getClientName(),mutingUser);
        Payload p = new Payload();
        p.setPayloadType(PayloadType.MUTE);
        p.setClientName(mutedUser.getClientName());
        p.setMutedClientID(mutedUser.getClientId());
        return send(p);
    }

    // UCID: jeo29
    // Date: April 27, 2024
    public boolean sendUnmuteUser(ServerThread unMutedUser, ServerThread mutingUser){
        if (!currentRoom.isUserMuted(unMutedUser.getClientName(),mutingUser.getClientName())) {
            return false; // Don't send mute notification if user is already muted
        }
        currentRoom.unmuteUser(unMutedUser.getClientName());
        Payload p = new Payload();
        p.setPayloadType(PayloadType.UNMUTE);
        p.setClientName(unMutedUser.getClientName());
        p.setMutedClientID(unMutedUser.getClientId());
    ...
    public boolean exportMessages() {
        Payload p = new Payload();
        p.setPayloadType(PayloadType.EXPORT);
        return send(p);
    }
